  ...
  try {
    ...
    if (action === "start") {
      if (!gateEvaluation.passed) {
        throw new Error(`Execution blocked: ${gateEvaluation.reasons.join(", ")}`);
      }
    ...
    if (action === "check") {
      ...
      const shouldTrade = gateEvaluation.passed && suggestedAllocation >= 0.08 && !hasCriticalAlert;
      ...
      const { data: order, error: orderError } = await supabase.from("live_orders").insert({
        ...
        error_message: shouldTrade ? null : hasCriticalAlert ? "Execution blocked by critical monitor alert." : gateEvaluation.passed ? "Allocation below trade threshold." : `Quality gate blocked execution: ${gateEvaluation.reasons.join(", ")}`,
      }).select().single();
      if (orderError) throw orderError;

      await supabase.from("live_portfolios").update({ last_signal_check: new Date().toISOString() }).eq("id", portfolio.id);
      return Response.json({ ok: true, order, executionMode, blocked: !shouldTrade, qualityGatePassed: gateEvaluation.passed, qualityGateReasons: gateEvaluation.reasons }, { headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
